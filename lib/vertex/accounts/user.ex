defmodule Vertex.Accounts.User do
  ...
  schema "users" do
    ...
    has_one :user_privilege_ruleset, Vertex.Accounts.UserPrivilegeRuleset, foreign_key: :user_id

    has_many :uploaded_avatars, Vertex.UserContent.Avatar, foreign_key: :uploader_id
    has_many :uploaded_maps, Vertex.UserContent.Map, foreign_key: :uploader_id
    has_many :uploaded_props, Vertex.UserContent.Prop, foreign_key: :uploader_id

    many_to_many :friendships, Vertex.UserRelations.Friendship,
      join_through: "friendships",
      join_keys: [user: :id, friend: :id]

    has_many :hosted_shards, Vertex.VSekai.Shard, foreign_key: :user_id

    has_many :identity_proofs_from, Vertex.UserRelations.IdentityProof, foreign_key: :user_from_id
    has_many :identity_proofs_to, Vertex.UserRelations.IdentityProof, foreign_key: :user_to_id
  ...
  def validate_username(changeset, field) when is_atom(field) do
    validate_change(changeset, field, fn _current_field, value ->
      if EmailChecker.valid?(value) or Vertex.Accounts.get_by_email(value) do
        [{field, " is not valid!"}]
      else
        []
      end
  ...
  def validate_email(changeset, field) when is_atom(field) do
    validate_change(changeset, field, fn _current_field, value ->
      if !EmailChecker.valid?(value) or Vertex.Accounts.get_by_username(value) do
        [{field, " is not valid!"}]
      else
        []
      end
