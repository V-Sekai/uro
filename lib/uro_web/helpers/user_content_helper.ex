defmodule UroWeb.Helpers.UserContentHelper do
  ...
  def get_api_user_content(user_content, config) do
    ...
    map = if Map.get(config, :merge_is_public, false) == true, do: Map.merge(map, %{is_public: user_content.is_public}), else: map
    map = if Map.get(config, :merge_inserted_at, false) == true, do: Map.merge(map, %{inserted_at: user_content.inserted_at}), else: map
    map = if Map.get(config, :merge_updated_at, false) == true, do: Map.merge(map, %{updated_at: user_content.updated_at}), else: map
    map = if Map.get(config, :merge_uploader_id, false) == true, do: Map.merge(map, %{uploader_id: user_content.uploader_id}), else: map
    map
  end
