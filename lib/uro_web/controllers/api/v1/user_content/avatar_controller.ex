defmodule UroWeb.API.V1.UserContent.AvatarController do
  ...
  def index(conn, _params) do
    ...
    conn
    |> put_status(200)
    |> json(%{data: %{avatars: UroWeb.Helpers.UserContentHelper.get_api_user_content_list(avatars, %{merge_uploader_id: true})}})
  end
    ...
    |> case do
      %Uro.UserContent.Avatar{} = avatar ->
        ...
        |> json(%{data: %{avatar: UroWeb.Helpers.UserContentHelper.get_api_user_content(
          avatar, %{merge_uploader_id: true, merge_is_public: true})}})
      _ ->
        conn
        |> put_status(400)
    end
