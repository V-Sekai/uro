defmodule VertexWeb.API.V1.SessionController do
  use VertexWeb, :controller
  use VertexWeb.Helpers.Auth
  alias VertexWeb.APIAuthPlug
  alias Plug.Conn
  ...
  defp session_data(conn, user) do
    ...
    |> json(%{
      data: %{
        ...
        user: user,
        user_privilege_ruleset: VertexWeb.Helpers.Auth.get_user_privilege_ruleset(user)
      }
    })
  end
  ...
  def create(conn, %{"user" => user_params}) do
    ...
    |> case do
      {:ok, conn} ->
        conn
        |> Vertex.EnsureUserNotLockedPlug.call(VertexWeb.APIAuthErrorHandler)
