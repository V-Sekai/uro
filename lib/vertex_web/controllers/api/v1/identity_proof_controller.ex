defmodule VertexWeb.API.V1.IdentityProofController do
  ...
  def create(conn, %{"identity_proof" => identity_proof_params}) do
    if !Map.has_key?(identity_proof_params, "user_to") do
      ...
      user_to = Vertex.Accounts.get_user!(Map.get(identity_proof_params, "user_to"))

      if user_to do
        user_from
        |> Vertex.UserRelations.create_identity_proof(user_to)
        |> case do
          {:ok, identity_proof} ->
  ...
  def show(conn, %{"id" => id}) do
    id
    |> Vertex.UserRelations.get_identity_proof_as!(conn.assigns[:current_user])
    |> case do
      nil ->
        conn
        |> put_status(400)
