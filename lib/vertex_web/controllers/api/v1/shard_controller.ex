defmodule VertexWeb.API.V1.ShardController do
  ...
  def ensure_user_is_current_user_or_nil(conn, params) do
    if VertexWeb.Helpers.Auth.signed_in?(conn) do
      Map.put(params, "user_id", VertexWeb.Helpers.Auth.get_current_user(conn).id)
    else
  ...
  def can_connection_modify_shard(conn, shard) do
    if shard.user != nil and
         VertexWeb.Helpers.Auth.signed_in?(conn) and
         shard.user == VertexWeb.Helpers.Auth.get_current_user(conn) do
      true
    else
      if shard.user == nil and
  ...
  def index(conn, _params) do
    ...
    conn
    |> put_status(200)
    |> json(%{data: %{shards: VertexWeb.Helpers.Shard.get_api_shard_list_public(shards)}})
  end
